...
for run in range(NUM_RUNS):
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if auc > best_auc:
            ...
            best_ap = ap
            patience_counter = 0
            torch.save(model.state_dict(), f"./fitkg_output/mlp_emb/best_mlp_run{run}.pt")
        else:
            patience_counter += 1
    ...
    # --------------------------------------------------------
    model.load_state_dict(torch.load(f"./fitkg_output/mlp_emb/best_mlp_run{run}.pt"))
    model.eval()
    ...
    torch.save(
        z.detach().cpu(),
        f"./fitkg_output/mlp_emb/mlp_embeddings_run{run}.pt"
    )

    print(f"Embeddings guardados: mlp_embeddings_run{run}.pt")
...
print("\nResultados guardados en mlp_experiment_results.csv")
...
# torch.save(z.detach().cpu(), "mlp_embeddings.pt")
# print("Embeddings guardados.")
