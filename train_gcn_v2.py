...
# ============================================================
# SPLIT ESTRATIFICADO (MISMO QUE R-GCN)
# ============================================================
...
# ============================================================
# FUNCTIONAL LOSS (IGUAL QUE R-GCN)
# ============================================================
...
def functional_loss(z, pos_pairs, neg_pairs, margin=0.3):
    ...
    pos_sim = torch.stack([(z[i] * z[j]).sum() for i, j in pos_pairs])
    neg_sim = torch.stack([(z[i] * z[j]).sum() for i, j in neg_pairs])
...
# Construir functional pairs SOLO con train edges
def build_exercise_muscle_map(edge_index, edge_type):
...
for run in range(NUM_RUNS):
    ...
    # --------------------------------------------------------
    # Set seeds (reproducible per run)
    # --------------------------------------------------------
    ...
    muscle_map = build_exercise_muscle_map(train_edge_index, train_edge_type)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        pos_scores = (z[train_edge_index[0]] * z[train_edge_index[1]]).sum(dim=1)
        neg_scores = (z[neg_edge_index[0]] * z[neg_edge_index[1]]).sum(dim=1)
        ...
        if auc > best_auc:
            ...
            best_ap = ap
            patience_counter = 0
            torch.save(model.state_dict(), f"best_gcn_run{run}.pt")
        else:
            patience_counter += 1
    ...
    # --------------------------------------------------------
    model.load_state_dict(torch.load(f"best_gcn_run{run}.pt"))
    model.eval()
    ...
    torch.save(
        z.detach().cpu(),
        f"./fitkg_output/gcn_emb/gcn_embeddings_run{run}.pt"
    )

    print(f"Embeddings guardados: gcn_embeddings_run{run}.pt")
    ...
    # torch.save(z.detach().cpu(), "gcn_embeddings.pt")
...
results_df.to_csv("gcn_experiment_results.csv", index=False)

print("\nResultados guardados en gcn_experiment_results.csv")